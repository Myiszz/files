...
app.get('/', (req, res) => {
    ...
    let zz = {username:"123",password:'efbb760482645423b68643b0ded8686e'}
    let num = 0;
    let num2 = 0;  let flg = true;
    let miao = 0;
    let flgTime = null;
    let webBody = function () {
        // if(!flg) return ;
        request({
            url: 'http://vps44.com/index.php?c=member&a=login&index=0',
            method: "POST",
            json: true,
            headers: {
            ...
            if (error) {
                console.log("失败" + num2 + '次,10s后重新运行.....');
                // flg = false
